...
const DEMO_WHATSAPP_DOCUMENT_LINK =
  'https://d2q3tfs05n2hft.cloudfront.net/2026-03-17T09%3A53%3A13.436Z-invoice.pdf';
...
invoicesRouter.post('/generate', express.json({ limit: '2mb' }), async (req, res) => {
  ...
  let invoicePdfLocation: string | null = null;
  let invoicePdfUpload: unknown = null;
  let invoicePdfUploadError: string | null = null;
  ...
  const whatsappPayload = {
    ...
    message: {
      ...
      template: {
        ...
        language: { policy: 'deterministic' as const, code: 'en' as const },
        components: [
          {
          ...
          },
          {
            type: 'body' as const,
            parameters: [
              { type: 'text' as const, text: firstName(gen.invoice.customerName) },
              { type: 'text' as const, text: gen.invoice.invoiceNumber },
              { type: 'text' as const, text: formatMinor(gen.invoice.totalMinor, gen.invoice.currency) },
              { type: 'text' as const, text: gen.invoice.dueAt.slice(0, 10) }
            ]
          }
        ]
      }
