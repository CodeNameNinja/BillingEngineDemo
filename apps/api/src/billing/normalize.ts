...
export function normalizeExtractedTerms(terms: ExtractedBillingTerms): NormalizationResult {
  ...
  const charges: NormalizedBillingModel['charges'] = [];
  ...
  const normalized: NormalizedBillingModel = {
    version: 1,
...
function normalizeFrequency(
  f: ExtractedBillingTerms['invoiceFrequency']
): NormalizedBillingModel['invoice']['frequency'] | null {
  if (!f || f === 'unknown') return null;
  return f;
}

function normalizeDueDateRule(input?: string): NormalizedBillingModel['invoice']['dueDateRule'] {
