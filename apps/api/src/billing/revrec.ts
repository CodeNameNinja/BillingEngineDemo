import type { RevRecSchedule } from '@demo/shared';

export function buildMonthlyRevRecSchedule(args: {
  invoiceId: string;
  currency: RevRecSchedule['currency'];
  ...
  const months: RevRecSchedule['months'] = [];
...
function addMonthsToYearMonth(yearMonth: string, deltaMonths: number) {
